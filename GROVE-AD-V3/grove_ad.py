#!/usr/bin/env python3
"""Standalone GROVE-AD V3 research implementation.
...
def load_series(path: Path) -> tuple[np.ndarray, np.ndarray, int]:
    ...
    label_column = label_candidates[-1] if label_candidates else frame.columns[-1]
    labels = frame[label_column].to_numpy(dtype=np.int64, copy=True)
    values = frame.drop(columns=[label_column]).to_numpy(dtype=np.float32, copy=True)
    ...
    # Official TSB-AD/PaAno training is label-blind and uses the complete
    # filename-declared ``tr_<N>`` prefix.  Official benchmark files may have
    # positive labels inside that declared prefix; labels remain untouched and
    # are used only by the evaluator after scoring.
    return values, labels, train_index
...
class GroveEncoder(nn.Module):
    """Dilated residual token encoder used by GROVE-AD."""

    def __init__(self, in_channels: int, config: GroveConfig) -> None:
        super().__init__()
        config.validate()
        self.config = config
    ...
    def encode_tokens(self, x: torch.Tensor) -> torch.Tensor:
        if x.ndim != 3:
            raise ValueError("GroveEncoder expects [batch, channels, time]")
    ...
    *,
    ...
    if save_checkpoint:
        torch.save(
            ...
            artifact / "grove_encoder.pt",
        )
    if save_scores:
        np.savez_compressed(
            artifact / "point_scores.npz",
            grove_gb_exemplar=point_score.astype(np.float32),
        )
    record = {
        "file": path.name,
...
def build_config(args: argparse.Namespace) -> GroveConfig:
    return GroveConfig(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    for position, filename in enumerate(files, 1):
        ...
        if args.resume and series_path.is_file():
            ...
            gb_row = next(
                method for method in record["methods"] if method["method"] == "grove_gb_exemplar"
            )
            print(
