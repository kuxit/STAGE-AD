...
- Baseline selection: paper-reported, author-released, or official TSB-AD
  settings; no additional baseline search. A documented stability default is
  allowed only when a matching setting does not exist and must be frozen before
  Eval.
- STAGE selection: official TSB-AD Tuning only, one configuration per subset,
  with the same search budget for every subset. Eval metrics must never affect
  configuration, including for Exathlon.
- Storage: full-precision metrics only; no checkpoint or score array

## Local readiness matrix (2026-07-22)

| Asset | Local status | Notes |
|---|---:|---|
| Frozen controller and common utilities | Ready | Hashes match |
| STAGE source | Ready | `STAGE/stage.py` matches the active checksum |
...
active entry points, and five pinned upstream trees. A
...
10. Complete admitted baselines first, with PaAno first on the GPU queue; run
    STAGE only after baseline completeness is verified.

## Result validity

A unit counts only when its JSON has seed 2026, `error` is null, and all six
