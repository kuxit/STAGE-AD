...
APP="${STAGE_ENCODER_APP:-/root/autodl-tmp/STAGE-AD-majority-encoder-search}"
FORMAL="${STAGE_FORMAL_REPO:-/root/autodl-tmp/STAGE-AD}"
PYTHON="${STAGE_PYTHON:-/root/autodl-tmp/envs/stage-py310/bin/python}"
RESULT_ROOT="${STAGE_ENCODER_RESULT_ROOT:-/root/autodl-tmp/results/STAGE_majority_encoder/e1_seed2026}"
PROTOCOL="$APP/configs/stage_majority_encoder_e1.json"
PRECOMPUTED_PLAN="$APP/configs/stage_majority_encoder_e1_plan.json"
...
(
  cd "$APP"
  sha256sum -c checksums/stage-majority-encoder-e1.sha256
)
...
echo "launching encoder E1 with ${workers_per_gpu} workers/GPU on ${gpu_count} GPU(s)"
