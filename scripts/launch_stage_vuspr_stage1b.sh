...
RESULT_ROOT="${STAGE_VUSPR_RESULT_ROOT:-/root/autodl-tmp/results/STAGE_vuspr_tuning/stage1b_seeds2027_2028}"
PROTOCOL="$APP/configs/stage_vuspr_stage1b.json"
PRECOMPUTED_PLAN="$APP/configs/stage_vuspr_stage1b_plan.json"
...
if [[ -n "$(git -C "$APP" status --porcelain)" ]]; then
  echo "refusing to run from a dirty STAGE VUS-PR Stage1B worktree" >&2
  exit 1
fi
...
echo "launching STAGE Stage1B with ${workers_per_gpu} workers/GPU on ${gpu_count} GPUs"
