...
APP="${STAGE_GEOMETRY_APP:-/root/autodl-tmp/STAGE-AD-majority-geometry-diagnosis}"
FORMAL="${STAGE_FORMAL_REPO:-/root/autodl-tmp/STAGE-AD}"
PYTHON="${STAGE_PYTHON:-/root/autodl-tmp/envs/stage-py310/bin/python}"
RESULT_ROOT="${STAGE_GEOMETRY_RESULT_ROOT:-/root/autodl-tmp/results/STAGE_majority_geometry_diagnosis/d1_seed2026}"
PROTOCOL="$APP/configs/stage_majority_geometry_d1.json"
PRECOMPUTED_PLAN="$APP/configs/stage_majority_geometry_d1_plan.json"
...
(
  cd "$APP"
  sha256sum -c checksums/stage-majority-geometry-d1.sha256
)
...
workers_per_gpu="${STAGE_WORKERS_PER_GPU:-8}"
...
echo "launching geometry D1 with ${workers_per_gpu} workers/GPU on ${gpu_count} GPU(s)"
